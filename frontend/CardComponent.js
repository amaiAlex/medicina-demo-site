...
function sendAppointmentData() {
  fetch('http://localhost:3000/appointments', {
    ...
    console.log('Отправлено на сервер:', chosenDoctor.doctor_id, chosenDayId, chosenTimeId)
}
...
function render() {
  ...
  if (currentCard === 'infoCard') {
        ...
        doctorsData.forEach(doctor => {
          let cardHtml = `
            <div class="col mb-4" id="doctor-${doctor.doctor_id}">
              <div class="card">
              <img src="${getDoctorImage(doctor.doctor_id)}" class="card-img-top" alt="${doctor.name}">
              <div class="card-body d-flex flex-column">
                  <p class="card-text mt-auto">${doctor.occupation}</p>
                  <h5 class="card-title">${doctor.doctor_name}</h5>
                  <a href="#" class="btn btn-primary make-appointment" onclick='chooseDoctor(${JSON.stringify(doctor)})'>Записаться на прием</a>
                </div>
              </div>
            </div>
          `;
    ...
    fetch(`http://localhost:3000/appointments?doctor_id=${chosenDoctor.doctor_id}`)
      .then(response => response.json())
      .then(appointments => {
        let cardHtml = `
          <div class="col mb-4">
        ...
        `;
        document.getElementById(`doctor-${chosenDoctor.doctor_id}`).innerHTML = cardHtml;
      })
      .catch(error => console.error('Error fetching appointments data:', error));
  } else if (currentCard === 'timeCard') {
    // Fetch appointments data for the selected doctor from the server
    fetch(`http://localhost:3000/appointments?doctor_id=${chosenDoctor.doctor_id}&day=${chosenDayId}`)
      .then(response => response.json())
      .then(appointments => {
        let cardHtml = `
          <div class="col mb-4">
              ...
              <div class="card-body">
                ...
                ${appointments
                  ...
                  .map(time => {
                    ...
                    const isAvailable = appointments.some(appointment => appointment.time === time && appointment.day === chosenDayId && appointment.doctor_id === chosenDoctor.doctor_id && appointment.available);
